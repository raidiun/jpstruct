...
describe('Test integers', function() {
    class IntTester {
        ...
        _test_one(x) {
            ...
            let view = new Uint8Array(got);
            [...view].should.be.eql(expected);
...
describe('Test signed/unsigned int64:', function() {
    ...
    it('unpack <Q', function() {
        var unpacked = jpstruct.unpack('<Q', new Uint8Array([0xff, 0xff, 0xe1, 0xff, 0xa0, 0xff, 0x00, 0x00]).buffer );
...
describe('Q Boundary tests:', function() {
    ...
    it('unpack <Q full', function() {
        let unpacked = jpstruct.unpack('<Q', new Uint8Array([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]).buffer );
        unpacked.length.should.be.eql(1);
    ...
    it('unpack <Q zero', function() {
        let unpacked = jpstruct.unpack('<Q', new Uint8Array([0, 0, 0, 0, 0, 0, 0, 0]).buffer);
        unpacked.length.should.be.eql(1);
    ...
    it('unpack <Q one', function() {
        let unpacked = jpstruct.unpack('<Q', new Uint8Array([1, 1, 1, 1, 1, 1, 1, 1]).buffer);
        unpacked.length.should.be.eql(1);
    ...
    it('unpack <Q 0xfe', function() {
        let unpacked = jpstruct.unpack('<Q', new Uint8Array([0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe, 0xfe]).buffer);
        unpacked.length.should.be.eql(1);
